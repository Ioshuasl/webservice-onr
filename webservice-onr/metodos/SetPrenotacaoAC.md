...
| 4 | Chamar `SetPrenotacaoAC` passando `Hash` + demais parâmetros |
...
_Listar campos na ordem de `<SetPrenotacaoAC_WSReq>` no WSDL local._
...
| — | Senha - Senha da prenotação a ser cadastrada (campo opcional, tipo string). | — | não | — | — |
...
- [`especificacao_wsoficio_dev.md`](../../especificacao_wsoficio_dev.md) — Envelope de Entrada/Saída `SetPrenotacaoAC`
