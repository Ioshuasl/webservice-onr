...
_Listar campos na ordem de `<SetContratoExigenciaAC_WSReq>` no WSDL local._
...
| `ExigenciaFinal` | Quando um contrato do tipo de serviço Exame/Cálculo, a exigência final corresponde ao último ato cadastrado (impossibilitando cadastro de novas exigências), informando inclusive o valor do contrato se alterado posteriormente para Registro/Averbação | boolean | — | — | — |
| `Resposta` | Resposta a ser adicionada na exigência do contrato | string | — | — | — |
| `Descricao` | Nome que descreve o arquivo | string | — | — | — |
| `URLArquivo` | URL do arquivo. O cartório precisa informar uma URL válida para download do arquivo anexado. Os arquivos informados serão colocados em uma fila e serão baixados posteriormente pelo sistema. O pedido não será efetivamente respondido antes que todos os arquivos sejam baixados. | string | — | — | — |
