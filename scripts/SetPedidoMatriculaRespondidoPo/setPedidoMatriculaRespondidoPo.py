...
PREFIX = "PENHORA_ONLINE_SET_PEDIDO_MATRICULA_RESPONDIDO_"
ANEXO_ITEM = "SetPedidoMatriculaRespondidoPO_Anexo_WSReq"
...
def main() -> int:
    ...
    print("=== Parâmetros SetPedidoMatriculaRespondidoPO ===")
    print(json.dumps(o_request, ensure_ascii=False, indent=2))
    print(f"\nEndpoint: {cfg['endpoint']}")

    result = call_operation_from_cfg(cfg, "SetPedidoMatriculaRespondidoPO", o_request)
    ...
    if not response.get("RETORNO"):
        ...
        print(
            f"\nSetPedidoMatriculaRespondidoPO falhou: [{codigo}] {descricao}",
            file=sys.stderr,
        )
    ...
    print(
        f"\nOK — Resposta registrada no pedido matrícula {cfg['id_pedido']} "
