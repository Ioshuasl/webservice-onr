/**
 * Lista pedidos para exportação v2 (ListPedidosExportacaoPO_v2) no webservice Penhora Online da ONR.
 */
...
const OPERATION = "ListPedidosExportacaoPO_v2";
...
async function listPedidosExportacaoPoV2(cfg, oRequest) {
  ...
  let response;
  try {
    [response] = await client.ListPedidosExportacaoPO_v2Async({ oRequest });
  } catch (err) {
  ...
  const result =
    response?.ListPedidosExportacaoPO_v2Result ??
    response?.ListPedidosExportacaoPO_v2Response?.ListPedidosExportacaoPO_v2Result ??
    response;
...
async function main() {
  ...
  const response = await listPedidosExportacaoPoV2(cfg, oRequest);
  console.log("\n=== Resposta ===");
  ...
  console.log(
    `\nOK — ${response.Pedidos.length} pedido(s) para exportação (v2: ValorDaDivida, ` +
      "Imovel[].PercentualExecutado / PercentualPenhorado)."
  );
}
