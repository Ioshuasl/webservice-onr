...
def build_request(hash_value: str, id_status: int) -> dict:
    return {"Hash": hash_value, "IDStatus": id_status}


def main() -> int:
    cfg = load_config()
    hash_value = resolve_auth_hash(cfg["chave"], cfg["login_cfg"])
    o_request = build_request(hash_value, cfg["id_status"])

    print("=== Parâmetros GetStatusAT ===")
    print(json.dumps(o_request, ensure_ascii=False, indent=2))
    print(f"\nEndpoint: {cfg['endpoint']}")

    result = call_operation_from_cfg(cfg, "GetStatusAT", o_request)
    ...
    if not response.get("RETORNO"):
        ...
        print(f"\nGetStatusAT falhou: [{codigo}] {descricao}", file=sys.stderr)
    ...
    print(
        f"\nOK — Status {cfg['id_status']}: protocolo {response.get('Protocolo', '—')}, "
        f"tipo {response.get('IDTipoStatus', '—')}, "
        f"descrição {response.get('DescricaoStatus', '—')}"
    )
    return 0
