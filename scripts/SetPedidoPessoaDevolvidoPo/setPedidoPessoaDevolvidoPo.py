...
PREFIX = "PENHORA_ONLINE_SET_PEDIDO_PESSOA_DEVOLVIDO_"
FIELD_ORDER = ("Hash", "IDPedido", "Resposta")
...
def load_config() -> dict:
    ...
    if not resposta:
        raise SystemExit(f"Defina {PREFIX}RESPOSTA no .env (motivo da devolução).")
...
def main() -> int:
    ...
    print("=== Parâmetros SetPedidoPessoaDevolvidoPO ===")
    print(json.dumps(o_request, ensure_ascii=False, indent=2))
    print(f"\nEndpoint: {cfg['endpoint']}")

    result = call_operation_from_cfg(cfg, "SetPedidoPessoaDevolvidoPO", o_request)
    ...
    if not response.get("RETORNO"):
        ...
        print(
            f"\nSetPedidoPessoaDevolvidoPO falhou: [{codigo}] {descricao}",
            file=sys.stderr,
        )
    ...
    print(
        f"\nOK — Pedido pessoa {cfg['id_pedido']} devolvido "
        f"(motivo registrado no ONR)."
    )
    return 0
