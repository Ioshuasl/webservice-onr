/**
 * Devolve pedido de certidão por pessoa (SetPedidoPessoaDevolvidoPO) no webservice Penhora Online da ONR.
 */
...
const PREFIX = "PENHORA_ONLINE_SET_PEDIDO_PESSOA_DEVOLVIDO_";
...
async function setPedidoPessoaDevolvidoPo(cfg, oRequest) {
  ...
  let response;
  try {
    [response] = await client.SetPedidoPessoaDevolvidoPOAsync({ oRequest });
  } catch (err) {
  ...
  return (
    response?.SetPedidoPessoaDevolvidoPOResult ??
    response?.SetPedidoPessoaDevolvidoPOResponse?.SetPedidoPessoaDevolvidoPOResult ??
    response
  );
}
...
async function main() {
  ...
  console.log("=== Parâmetros SetPedidoPessoaDevolvidoPO ===");
  console.log(JSON.stringify(oRequest, null, 2));
  console.log(`\nEndpoint: ${cfg.endpoint}`);

  const response = await setPedidoPessoaDevolvidoPo(cfg, oRequest);
  console.log("\n=== Resposta ===");
  console.log(JSON.stringify(response, null, 2));

  if (!response.RETORNO) {
    console.error(
      `\nSetPedidoPessoaDevolvidoPO falhou: [${response.CODIGOERRO}] ${response.ERRODESCRICAO}`
    );
    const hint =
  ...
  console.log(
    `\nOK — Pedido pessoa ${cfg.idPedido} devolvido (motivo registrado no ONR).`
  );
}
