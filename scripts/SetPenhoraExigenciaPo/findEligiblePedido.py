#!/usr/bin/env python3
"""Lista pedidos Penhora e encontra IDPedido elegível para SetPenhoraExigenciaPO."""
...
from lib.onr_penhora_exigencia import validate_pedido_for_exigencia  # noqa: E402
...
def main() -> int:
    ...
    for status, count in sorted(by_status.items()):
        ...
        print(f"  {status} ({label}): {count}", file=sys.stderr)
    ...
    for pid in sorted(pedidos):
        ...
        warnings = validate_pedido_for_exigencia(detail)
        entry = {
            "IDPedido": pid,
        ...
        elif detail.get("IDStatus") in (9, 10, 11, 1, 12):
            near_miss.append({**entry, "warnings": warnings})

    # Preferir pedidos ainda sem nota de exigência (9/10/11) em vez de status 7 (já em exigência)
    preferidos = [e for e in eligible if e.get("IDStatus") in (9, 10, 11)]
    ...
    if recomendado:
        best = recomendado
        print(
            f"\n>>> Sugestão .env: PENHORA_ONLINE_ID_PEDIDO={best['IDPedido']} "
            f"({best['Protocolo']}, status {best['IDStatus']} {best['Status']})",
            file=sys.stderr,
        )
        return 0

    print("\nNenhum pedido elegível encontrado no período.", file=sys.stderr)
    if near_miss:
        print("Candidatos próximos (com avisos):", file=sys.stderr)
