...
| Operação SOAP | `SetBaixaBoletoAC` |
...
| 4 | Chamar `SetBaixaBoletoAC` passando `Hash` + demais parâmetros |
...
_Listar campos na ordem de `<SetBaixaBoletoAC_WSReq>` no WSDL local._
...
| `IDBoleto` | ID do boleto obtido da listagem de boletos | int | — | — | — |
| `Convenio` | Indicar se o boleto foi gerado por um convênio, obtido através da listagem anterior de boletos | boolean | — | — | — |
...
- [`especificacao_wsoficio_dev.md`](../../especificacao_wsoficio_dev.md) — Envelope de Entrada/Saída `SetBaixaBoletoAC`
