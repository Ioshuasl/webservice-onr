...
@workflow({
    id: 'Vv0AvxZGIBLkMrkS',
    name: 'List Instituicoes OE',
    active: false,
    isArchived: false,
    settings: { executionOrder: 'v1', availableInMCP: false, callerPolicy: 'workflowsFromSameOwner' },
})
export class ListInstituicoesOeWorkflow {
    // =====================================================================
    // CONFIGURATION DES NOEUDS
    // =====================================================================

    @node({
        id: '6b6c7d8e-9f0a-4b1c-8d2e-3f4a5b6c7d8e',
        webhookId: '91f0d4c2-7a6b-4e5f-9c81-2d3e4f5a6b7c',
        name: 'Webhook',
    ...
    Webhook = {
        httpMethod: 'POST',
        path: '91f0d4c2-7a6b-4e5f-9c81-2d3e4f5a6b7c',
    ...
    @node({
        id: '7c8d9e0f-a1b2-4c3d-9e4f-5a6b7c8d9e0f',
 ...
 `,
    ...
    @node({
        id: '8d9e0f1a-b2c3-4d4e-8f5a-6b7c8d9e0f1a',
...
return [{
    ...
    @node({
        id: '9e0f1a2b-c3d4-4e5f-9a6b-7c8d9e0f1a2b',
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    <tns:ListInstituicoesOE xmlns:tns="http://tempuri.org/WSOficio">
      <tns:oRequest>
        <tns:Hash>\${escapeXml(item.hash)}</tns:Hash>
      </tns:oRequest>
    </tns:ListInstituicoesOE>
  </soap:Body>
</soap:Envelope>\`;
...
const extrairInstituicoes = (origem) => {
    const instituicoes = [];
    const regex = /<(?:\\w+:)?ListInstOE_Inst_WSResp[^>]*>([\\s\\S]*?)<\\/(?:\\w+:)?ListInstOE_Inst_WSResp>/g;
    let match;

    while ((match = regex.exec(origem)) !== null) {
        const bloco = match[1];
        instituicoes.push({
            id_instituicao: parseInt(extrairTag('IDInstituicao', bloco), 10) || 0,
...
return {
    json: {
        ...
        codigo_erro,
        mensagem_erro,
        dados: sucesso ? { instituicoes: extrairInstituicoes(xml) } : dadosVazio()
    }
};`,
    };
