...
// <workflow-map>
// Workflow : [AUTORIB-32] (rib) DetalheCobranca - RIB
// Nodes   : 7  |  Connections: 7
//
// NODE INDEX
// ──────────────────────────────────────────────────────────────────
// Property name                    Node type (short)         Flags
// ReceiveRibDetalheCobranca          webhook                    [creds]
// ValidarEntrada                     code
// EntradaValida                      if
// RespostaErroEntrada                code
// DetalheCobrancaRib                 httpRequest                [onError→regular]
// BuildDetalheCobrancaResponse       code
// ReturnDetalheCobrancaResponse      respondToWebhook
//
// ROUTING MAP
// ──────────────────────────────────────────────────────────────────
// ReceiveRibDetalheCobranca
//    → ValidarEntrada
//      → EntradaValida
//        → DetalheCobrancaRib
//          → BuildDetalheCobrancaResponse
//            → ReturnDetalheCobrancaResponse
//       .out(1) → RespostaErroEntrada
//          → ReturnDetalheCobrancaResponse (↩ loop)
// </workflow-map>
...
@workflow({
    id: 'vdX5HRRokKzYiMEn',
    name: '[AUTORIB-32] (rib) DetalheCobranca - RIB',
    active: false,
    isArchived: false,
    settings: { executionOrder: 'v1', availableInMCP: false, callerPolicy: 'workflowsFromSameOwner' },
})
export class Autorib32RibDetalhecobrancaRibWorkflow {
    // =====================================================================
    // CONFIGURATION DES NOEUDS
    // =====================================================================

    @node({
        id: 'a122b1c2-d3e4-5f6a-7b8c-9d0e1f2a3b4c',
        webhookId: 'b122c3d4-e5f6-a7b8-c9d0-e1f2a3b4c5d6e',
        name: 'Receive RIB Detalhe Cobranca',
    ...
    @node({
        id: 'c122d4e5-f6a7-8b9c-0d1e-2f3a4b5c6d7e',
...
return [{
  json: {
    ...
    meta: {
      ...
      source: 'n8n-rib-detalhe-cobranca',
    },
    errorDefaultMsg: 'Erro ao obter detalhe da cobranca na API RIB.',
  },
}];
`,
    };
    ...
    @node({
        id: 'd122e5f6-a7b8-9c0d-1e2f-3a4b5c6d7e8f',
        name: 'Entrada valida?',
    ...
    EntradaValida = {
        conditions: {
            ...
            conditions: [
                {
                    id: 'cond-rib-detalhe-cobranca-valido',
    ...
    @node({
        id: 'e122f6a7-b8c9-0d1e-2f3a-4b5c6d7e8f9a',
    ...
    @node({
        id: 'f122a7b8-c9d0-1e2f-3a4b-5c6d7e8f9a0b',
        name: 'Detalhe Cobranca RIB',
    ...
    @node({
        id: 'a122b8c9-d0e1-2f3a-4b5c-6d7e8f9a0b1c',
        name: 'Build Detalhe Cobranca Response',
        type: 'n8n-nodes-base.code',
        version: 2,
        position: [1060, 180],
    })
...
return [{
  json: {
    ...
    response: {
      sucesso: true,
      hash_cobranca: entrada.hashCobranca ?? data.hash ?? null,
      status: data.status ?? null,
      data_status: data.dataStatus ?? null,
      data_geracao: data.dataGeracao ?? null,
      url_pagamento: data.url ?? null,
      valor_total: data.valorTotal ?? null,
      tipo_cobranca: data.tipoCobranca ?? null,
      data_vencimento: data.dataVencimento ?? null,
      observacao: data.observacao ?? null,
      identificador_cliente: data.identificadorCliente ?? null,
      dados_pagador: data.dadosPagador ?? null,
      servicos: data.servicos ?? [],
      devolucoes: data.devolucoes ?? [],
      pagamento_vinculado: data.pagamentoVinculado ?? null,
      detalhe: data,
      ambiente: meta.ambiente ?? null,
      meta,
    },
...
`,
    ...
    @node({
        id: 'b122c9d0-e1f2-3a4b-5c6d-7e8f9a0b1c2d',
        name: 'Return Detalhe Cobranca Response',
    ...
    @links()
    defineRouting() {
        this.ReceiveRibDetalheCobranca.out(0).to(this.ValidarEntrada.in(0));
        this.ValidarEntrada.out(0).to(this.EntradaValida.in(0));
        this.EntradaValida.out(0).to(this.DetalheCobrancaRib.in(0));
        this.EntradaValida.out(1).to(this.RespostaErroEntrada.in(0));
        this.DetalheCobrancaRib.out(0).to(this.BuildDetalheCobrancaResponse.in(0));
        this.BuildDetalheCobrancaResponse.out(0).to(this.ReturnDetalheCobrancaResponse.in(0));
        this.RespostaErroEntrada.out(0).to(this.ReturnDetalheCobrancaResponse.in(0));
    }
}
