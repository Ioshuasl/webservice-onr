...
// <workflow-map>
// Workflow : [AUTSEETJGO-18] (see tjgo) DistribuicaoAtosListar - Pedido de Atos
// Nodes   : 7  |  Connections: 7
//
// NODE INDEX
// ──────────────────────────────────────────────────────────────────
// Property name                    Node type (short)         Flags
// ReceiveSeeTjgoDistribuicaoAtosListar webhook                    [creds]
// ValidarEntrada                     code
// EntradaValida                      if
// RespostaErroEntrada                code
// DistribuicaoAtosListarSeeTjgo      httpRequest                [onError→regular]
// BuildDistribuicaoAtosListarResponse code
// ReturnDistribuicaoAtosListarResponse respondToWebhook
//
// ROUTING MAP
// ──────────────────────────────────────────────────────────────────
// ReceiveSeeTjgoDistribuicaoAtosListar
//    → ValidarEntrada
//      → EntradaValida
//        → DistribuicaoAtosListarSeeTjgo
//          → BuildDistribuicaoAtosListarResponse
//            → ReturnDistribuicaoAtosListarResponse
//       .out(1) → RespostaErroEntrada
//          → ReturnDistribuicaoAtosListarResponse (↩ loop)
// </workflow-map>
...
@workflow({
    id: 'Vyab6KWzOS3TXXT2',
    name: '[AUTSEETJGO-18] (see tjgo) DistribuicaoAtosListar - Pedido de Atos',
    active: false,
    isArchived: false,
    settings: { executionOrder: 'v1', availableInMCP: false, callerPolicy: 'workflowsFromSameOwner' },
})
export class Autseetjgo18SeeTjgoDistribuicaoatoslistarPedidoDeAtosWorkflow {
    // =====================================================================
    // CONFIGURATION DES NOEUDS
    // =====================================================================

    @node({
        id: 'e1f20018-0001-4000-8000-000000000001',
        webhookId: 'e1f20018-0001-4000-8000-000000000011',
        name: 'Receive SEE TJGO Distribuicao Atos Listar',
    ...
    @node({
        id: 'e1f20018-0001-4000-8000-000000000002',
...
const listarUrl = baseUrl + '/distribuicao_de_atos' + (queryString ? '?' + queryString : '');
...
return [{
  json: {
    ...
    meta: {
      ...
      source: 'n8n-see-tjgo-distribuicao-atos-listar',
    },
    errorDefaultMsg: 'Erro ao listar lotes de distribuicao de atos na API SEE TJGO.',
  },
}];
`,
    };
    ...
    @node({
        id: 'e1f20018-0001-4000-8000-000000000003',
        name: 'Entrada valida?',
    ...
    EntradaValida = {
        conditions: {
            ...
            conditions: [
                {
                    id: 'cond-see-distribuicao-atos-listar-valido',
    ...
    @node({
        id: 'e1f20018-0001-4000-8000-000000000004',
...
`,
    ...
    @node({
        id: 'e1f20018-0001-4000-8000-000000000005',
        name: 'Distribuicao Atos Listar SEE TJGO',
    ...
    @node({
        id: 'e1f20018-0001-4000-8000-000000000006',
        name: 'Build Distribuicao Atos Listar Response',
        type: 'n8n-nodes-base.code',
        version: 2,
        position: [1060, 180],
    })
...
function extractDistribuicaoAtos(data) {
  ...
  return [{
    json: {
      ...
      response: {
        ...
        mensagem_erro: formatMensagens(parsed) ?? 'A API SEE TJGO rejeitou a listagem de lotes de distribuicao de atos.',
        erros: Array.isArray(parsed) ? parsed : null,
        sistema: 'SEE TJGO',
        status_http: statusCode,
        ambiente: meta.ambiente ?? null,
...
const distribuicaoAtos = extractDistribuicaoAtos(httpResult);
if (distribuicaoAtos === null) {
  return [{
    json: {
      statusCode: 502,
      response: {
        ...
        mensagem_erro: 'Resposta da API SEE TJGO em formato nao reconhecido (esperado array de lotes).',
        sistema: 'SEE TJGO',
        status_http: 502,
        ambiente: meta.ambiente ?? null,
        meta,
...
const response = {
  sucesso: true,
  status_http: 200,
  total_distribuicao_atos: distribuicaoAtos.length,
  distribuicao_atos: distribuicaoAtos,
...
`,
    ...
    @node({
        id: 'e1f20018-0001-4000-8000-000000000007',
        name: 'Return Distribuicao Atos Listar Response',
    ...
    @links()
    defineRouting() {
        this.ReceiveSeeTjgoDistribuicaoAtosListar.out(0).to(this.ValidarEntrada.in(0));
        this.ValidarEntrada.out(0).to(this.EntradaValida.in(0));
        this.EntradaValida.out(0).to(this.DistribuicaoAtosListarSeeTjgo.in(0));
        this.EntradaValida.out(1).to(this.RespostaErroEntrada.in(0));
        this.DistribuicaoAtosListarSeeTjgo.out(0).to(this.BuildDistribuicaoAtosListarResponse.in(0));
        this.BuildDistribuicaoAtosListarResponse.out(0).to(this.ReturnDistribuicaoAtosListarResponse.in(0));
        this.RespostaErroEntrada.out(0).to(this.ReturnDistribuicaoAtosListarResponse.in(0));
    }
}
