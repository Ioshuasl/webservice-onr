...
export function loadLoginConfig() {
  const autoLogin = ["1", "true", "yes"].includes(
    (process.env.CERTIDOES_AUTO_LOGIN || "true").toLowerCase()
  );
...
export function loadCertidoesSoapConfig() {
  return {
    wsdlPath:
      process.env.CERTIDOES_WSDL_PATH || path.join(ROOT, "wsdl", "certidoes.wsdl"),
    endpoint:
      process.env.CERTIDOES_ENDPOINT ||
      "https://hml3-wsoficio.onr.org.br/Certidoes.asmx",
  };
}
...
export async function resolveAuthHash(chave, loginCfg, { tokenIndex } = {}) {
  ...
  if (!loginCfg) {
    throw new Error(
      "Defina ONR_HASH_OVERRIDE ou CERTIDOES_AUTO_LOGIN=true " +
