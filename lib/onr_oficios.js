...
export function loadLoginConfig() {
  const autoLogin = ["1", "true", "yes"].includes(
    (process.env.OFICIOS_AUTO_LOGIN || "true").toLowerCase()
  );
...
export function loadOficiosSoapConfig() {
  return {
    wsdlPath:
      process.env.OFICIOS_WSDL_PATH || path.join(ROOT, "wsdl", "oficios.wsdl"),
    endpoint:
      process.env.OFICIOS_ENDPOINT ||
      "https://hml3-wsoficio.onr.org.br/oficios.asmx",
  };
}

export async function resolveAuthHash(chave, loginCfg, { tokenIndex } = {}) {
  const hashOverride = envStr("ONR_HASH_OVERRIDE");
  if (hashOverride) return hashOverride;

  if (!loginCfg) {
    throw new Error(
      "Defina ONR_HASH_OVERRIDE ou OFICIOS_AUTO_LOGIN=true " +
  ...
  const index =
    tokenIndex !== undefined ? tokenIndex : envInt("ONR_HASH_TOKEN_INDEX", 0);
  const token = pickToken(tokens, index);
  const hash = computeOnrAuthHash(chave, token);
  console.log(
    `Token usado: ${token} (índice ${index}, ${tokens.length} disponíveis)`
  );
...
export function hashErrorHint(codigoErro) {
  ...
  if (codigoErro === 51) {
    return "Não foi possível obter os dados (instituições ou pedido).";
  }
  if (codigoErro === 56) {
    return "Usuário sem permissão para acessar o pedido informado.";
  }
  if (codigoErro === 12) {
    return "IDPedido inválido ou inexistente.";
  }
  return null;
}
