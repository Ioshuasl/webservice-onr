"""Configuração e Hash compartilhados — webservice Ofícios (OE)."""
...
def load_login_config() -> dict | None:
    auto_login = os.getenv("OFICIOS_AUTO_LOGIN", "true").lower() in (
        "1",
        "true",
        "yes",
    )
...
def load_oficios_soap_config() -> dict:
    return {
        "wsdl_path": os.getenv(
            "OFICIOS_WSDL_PATH",
            str(ROOT / "wsdl" / "oficios.wsdl"),
        ),
        "endpoint": os.getenv(
            "OFICIOS_ENDPOINT",
