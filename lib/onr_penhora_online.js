...
export function loadLoginConfig() {
  const autoLogin = ["1", "true", "yes"].includes(
    (process.env.PENHORA_ONLINE_AUTO_LOGIN || "true").toLowerCase()
  );
...
export function loadPenhoraOnlineSoapConfig() {
  return {
    wsdlPath:
      process.env.PENHORA_ONLINE_WSDL_PATH ||
      path.join(ROOT, "wsdl", "penhoraonline.wsdl"),
    endpoint:
      process.env.PENHORA_ONLINE_ENDPOINT ||
      "https://hml3-wsoficio.onr.org.br/penhoraonline.asmx",
  };
}
...
export async function resolveAuthHash(chave, loginCfg, { tokenIndex } = {}) {
  ...
  if (!loginCfg) {
    throw new Error(
      "Defina ONR_HASH_OVERRIDE ou PENHORA_ONLINE_AUTO_LOGIN=true " +
