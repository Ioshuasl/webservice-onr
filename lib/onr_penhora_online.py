"""Configuração e Hash compartilhados — webservice Penhora Online."""
...
def load_login_config() -> dict | None:
    auto_login = os.getenv("PENHORA_ONLINE_AUTO_LOGIN", "true").lower() in (
        "1",
        "true",
        "yes",
    )
...
def load_penhora_online_soap_config() -> dict:
    return {
        "wsdl_path": os.getenv(
            "PENHORA_ONLINE_WSDL_PATH",
            str(ROOT / "wsdl" / "penhoraonline.wsdl"),
        ),
        "endpoint": os.getenv(
            "PENHORA_ONLINE_ENDPOINT",
