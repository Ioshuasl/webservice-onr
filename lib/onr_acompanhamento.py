"""Configuração e Hash compartilhados — webservice Acompanhamento de Títulos."""
...
def load_login_config() -> dict | None:
    auto_login = os.getenv("ACOMPANHAMENTO_TITULOS_AUTO_LOGIN", "true").lower() in (
        "1",
        "true",
        "yes",
    )
...
def load_acompanhamento_soap_config() -> dict:
    return {
        "wsdl_path": os.getenv(
            "ACOMPANHAMENTO_TITULOS_WSDL_PATH",
            str(ROOT / "wsdl" / "acompanhamentotitulos.wsdl"),
        ),
        "endpoint": os.getenv(
            "ACOMPANHAMENTO_TITULOS_ENDPOINT",
            "https://hml3-wsoficio.onr.org.br/acompanhamentotitulos.asmx",
        ),
    }


def resolve_auth_hash(chave: str, login_cfg: dict | None) -> str:
    hash_override = env_str("ONR_HASH_OVERRIDE")
    if hash_override:
