...
export function loadLoginConfig() {
  const autoLogin = ["1", "true", "yes"].includes(
    (process.env.BDLIGHT_AUTO_LOGIN || "true").toLowerCase()
  );
...
export function loadBdlightSoapConfig() {
  return {
    wsdlPath:
      process.env.BDLIGHT_WSDL_PATH || path.join(ROOT, "wsdl", "bdlight.wsdl"),
    endpoint:
      process.env.BDLIGHT_ENDPOINT ||
