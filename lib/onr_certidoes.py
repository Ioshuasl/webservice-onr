"""Configuração e Hash compartilhados — webservice Certidões a Emitir."""
...
def load_login_config() -> dict | None:
    auto_login = os.getenv("CERTIDOES_AUTO_LOGIN", "true").lower() in (
        "1",
        "true",
        "yes",
    )
...
def load_certidoes_soap_config() -> dict:
    return {
        "wsdl_path": os.getenv(
            "CERTIDOES_WSDL_PATH",
            str(ROOT / "wsdl" / "certidoes.wsdl"),
        ),
        "endpoint": os.getenv(
            "CERTIDOES_ENDPOINT",
